...
const ClientList = () => {
  ...
  const fetchClients = async () => {
    try {
      const response = await adminRequest.get(`${BASE_URL}/client/list`);
      setRows(response.data.data);
    } catch (error) {
      console.log("Client list not found");
    }
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          setRows((prevRows) =>
            prevRows.filter((client) => client.uuid !== clientUuid)
          );
        } catch (error) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setRows((prevRows) =>
        prevRows.map((client) =>
          client.uuid === editClient.uuid ? { ...client, ...formData } : client
        )
      );
            ...
            <tbody>
              {rows.length > 0 ? (
                rows.map((client) => (
                  ...
                  >
                    <td className="p-3 text-sm">{client.fullName}</td>
                    <td className="p-3 text-sm">{client.email}</td>
                    <td className="p-3 text-sm">{client.mobileNumber}</td>
            ...
            <div className="bg-white rounded-2xl shadow-lg max-w-md w-full p-6 relative">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p>{selectedClient.mobileNumber || "N/A"}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="bg-white rounded-2xl shadow-lg max-w-md w-full p-6 relative">
              ...
              <div className="flex flex-col items-center text-center mb-6">
                <div className="w-24 h-24 rounded-full overflow-hidden mb-3">
                  <img
                    src={
                      editClient.profileImageUrl ||
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(
                        editClient.fullName
                      )}&background=random`
                    }
                    alt={editClient.fullName}
                    className="object-cover w-full h-full"
                  />
                </div>
                <h3 className="text-xl font-semibold">{editClient.fullName}</h3>
                <p className="text-sm text-gray-500">{editClient.email}</p>
              </div>
              <form
                onSubmit={handleEditSubmit}
                className="grid grid-cols-2 gap-4 text-sm text-gray-700"
              >
                <div className="bg-gray-50 rounded-lg p-3 flex flex-col col-span-2">
